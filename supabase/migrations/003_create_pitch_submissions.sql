-- Create pitch_submissions table for artist pitch form
...
CREATE INDEX idx_pitch_submissions_release_date ON pitch_submissions(release_date);
CREATE INDEX idx_pitch_submissions_release_stage ON pitch_submissions(release_stage);
...
-- Create policy for service role to update (for dashboard management)
CREATE POLICY "Service role can update pitch submissions"
  ON pitch_submissions
  FOR UPDATE
  TO service_role
  USING (true)
...
-- Create policy for service role to delete (for dashboard management)
CREATE POLICY "Service role can delete pitch submissions"
  ON pitch_submissions
  FOR DELETE
  TO service_role
  USING (true);